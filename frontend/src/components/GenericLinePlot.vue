  ...
  methods: {
    ...
    moveTooltip: function (e) {
      ...
      tooltip
          ...
          .style("top", d3.pointer(e, this.svg.node())[1] + 70 + "px");
    },
    ...
    drawPlot: function (aData, aPoints, aProperties, aParameters) {
      //Compute scaling parameters based on the width of the container
      const scalingFactor = this.containerWidth ? this.containerWidth/this.minWidth : 1;
      ...
      var width = (this.minWidth - margin.left - margin.right) * scalingFactor;
      var height = (this.minHeight - margin.top - margin.bottom) * scalingFactor;
      ...
      var svg = d3
          ...
          .attr("transform", `scale(${this.scaleSVG})`);
      ...
      if (this.bShowCurve) {
        ...
        if (typeof aProperties[0].lineTooltip !== 'undefined') {
          invisiblePath
        ...
        if (!this.hideErrorBars && aParameters[0] && (aParameters[0]["ED50, inflection"] || aParameters[0]["inflection"] || aParameters[0]["pEC50"])) {
          var errorGroup = svg
              .selectAll("g.error")
              ...
              .data(
                  aParameters.map(function (x, i) {
                    let iterator = aProperties.length === 1 ? 0 : i;
                    let o = {
                      data: x["ED50, inflection"] || x["inflection"] || {value: Math.pow(10,-x["pEC50"].value), std_error:x["pEC50"].std_error} ,
      ...
      for (var i = 0; i < aPoints.length; i++) {
        if (aPoints[i].length > 0) {
          ...
          if (this.drawLine) {
            ...
            //Tooltips work only for ddPTM curves right now; we can identify them by checking whether they have a drugName
            if (typeof aProperties[0].drugName !== 'undefined') {
              invisibleLine
                  .attr("lineTooltip", function (d, i) {
                    let iterator = aProperties.length === 1 ? 0 : i;
                    return aProperties[iterator].lineTooltip;
            ...
            //Legacy: I think this was copy-pasted erroneously and was not noticed bc it was never true.
            //But: invisiblePath is not defined here anymore, so it will throw an error
            // else {
            //   invisiblePath
            //       .on("mouseenter", function () {
            //         var sId = d3.select(this).attr("id").split("-")[1]; // next time: we do data binding correctly
          ...
          svg
              ...
              .on("mouseenter", function (e, d) {
                ...
                that.showTooltip(dotTooltipTitle + `x=${that.exponentialX ? d.x.toExponential(1) : parseFloat(d.x.toPrecision(2))}, y=${d.y.toPrecision(4)} </pre>`)
              })
              .on("mousemove", function (e) {
                that.moveTooltip(e)
              })
      ...
      svg
          ...
          .attr("x", 0 - height / 2)
          .text(
              aProperties[0].responseUnit || this.yAxisLabel
          );
      ...
      if (!this.hideLegend) {
        ...
        var legend = svg
            ...
            .attr("transform", (d, i) => {
              ...
              var x = this.legendAtBottom ? -50 : width + LegendOffset * 2;
    ...
    init: function () {
      ...
      if (aParameters) {
        ...
        for (var i = 0; i < aParameters.length; i++) {
          ...
          if (this.kinobeads) {
            this.oErrorbarInfo = aParameters[i]["ED50, inflection"] || aParameters[i]["pEC50"];
